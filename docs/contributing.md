...
This monorepo keeps ten independent Pi packages under `packages/`:
...
Each package declares its Pi entry point in `package.json`. The root manifest loads all ten; each package also works on its own.
