 ...
 * 3c891a9640f80c271ccc666ab7a39f9811bc3fb6. Modified for @whamp/pi-btw.
...
function buildSeedMessages(ctx: ExtensionContext, thread: BtwDetails[]): Message[] {
	...
	try {
		...
		// Context seeding is optional; preserve the explicitly empty seed.
		seed.length = 0;
	}
...
/** Registers the persistent in-session `/btw` tangent overlay. */
export function registerBtwOverlay(pi: ExtensionAPI): void {
	...
	async function disposeSideSession(): Promise<void> {
		...
		try {
			...
			// Cleanup is best effort; explicitly discard unsubscribe failures.
			void error;
		}
		...
		try {
			...
			// Cleanup is best effort; explicitly discard abort failures.
			void error;
		}
	...
	async function summarizeThread(ctx: ExtensionContext, items: BtwDetails[]): Promise<string> {
		...
		try {
			...
			try {
				...
				// Temporary-session teardown is best effort.
				void error;
			}
			session.dispose();
		}
