...
interface BreakdownAnalysisState {
  error?: Error;
}
...
class BreakdownComponent implements Component {
  ...
  render(width: number): string[] {
    ...
    if (this.cachedWidth === width && this.cachedRows === termRows && this.cachedLines)
...
/** Registers the read-only `/session-breakdown` usage analytics command. */
export default function sessionBreakdownExtension(pi: ExtensionAPI) {
  pi.registerCommand("session-breakdown", {
    ...
    handler: async (_args, ctx: ExtensionContext) => {
      ...
      let aborted = false;
      const analysisState: BreakdownAnalysisState = {};
      const data = await ctx.ui.custom<BreakdownData | null>((tui, theme, _kb, done) => {
        ...
        void computeBreakdown(loader.signal)
          .then((breakdown) => {
            if (!aborted) done(breakdown);
          })
          .catch((error: unknown) => {
            analysisState.error = error instanceof Error ? error : new Error(String(error));
      ...
      if (!data) {
        ctx.ui.notify(
          aborted ? "Cancelled" : (analysisState.error?.message ?? "Failed to analyze sessions"),
          aborted ? "info" : "error",
        );
        return;
      }
