 ...
 * ~/.pi/history/<project>-<path-hash>/todos, or the path in PI_TODO_PATH). Each todo is a
...
import { getTodosDir, getTodosDirLabel, migrateLegacyTodosDir } from "./todo-storage-path.ts";
...
export default function todosExtension(pi: ExtensionAPI) {
	pi.on("session_start", async (_event, ctx) => {
		const todosDir = await migrateLegacyTodosDir(ctx.cwd);
	...
	pi.registerCommand("todos", {
		description: "List todos from the current project's global Pi history directory",
