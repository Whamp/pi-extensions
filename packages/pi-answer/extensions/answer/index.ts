...
import { type ExtensionAPI, type ExtensionContext, type ModelRegistry, BorderedLoader } from "@earendil-works/pi-coding-agent";
import {
	type Component,
	Editor,
	type EditorTheme,
...
async function extractQuestions(
	...
	let lastError = "Failed to extract questions";
	for (const model of models) {
		...
		if (response.stopReason === "error") {
			lastError = response.errorMessage?.trim() || `Failed to extract questions using ${model.id}`;
			continue;
		}
		...
		lastError = `${model.id} returned invalid question JSON`;
	}
...
class QnAComponent implements Component {
	...
	constructor(
		...
		const editorTheme: EditorTheme = {
			...
			selectList: {
				...
				scrollInfo: (s: string) => this.gray(s),
				noMatch: (s: string) => this.gray(s),
			},
		};
	...
	render(width: number): string[] {
		...
		const padToWidth = (line: string): string => padAnswerBoxLine(line, width, visibleWidth);
		...
		if (q.recommendation) {
			lines.push(padToWidth(emptyBoxLine()));
			lines.push(padToWidth(boxLine(this.yellow(this.bold("Recommendation:")))));
			const recommendationLines = wrapTextWithAnsi(q.recommendation, contentWidth - 4);
			for (const recommendationLine of recommendationLines) {
				lines.push(padToWidth(boxLine(this.yellow(recommendationLine))));
			}
		}
...
export default function (pi: ExtensionAPI) {
	const answerHandler = async (ctx: ExtensionContext) => {
			...
			const extractionResult = await ctx.ui.custom<ExtractionResult | null>((tui, theme, _kb, done) => {
				...
				extractQuestions(lastAssistantText!, extractionModels, ctx.modelRegistry, loader.signal)
					.then(done)
					.catch((error) => {
