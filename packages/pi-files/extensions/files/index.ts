 ...
 * 3c891a9640f80c271ccc666ab7a39f9811bc3fb6. Modified for this Pi setup.
 * Licensed under Apache-2.0; see LICENSE.agent-stuff in this directory.
 */
/**
 * Files Extension
 *
...
const showActionSelector = async (
  ctx: ExtensionContext,
  options: { canQuickLook: boolean; canEdit: boolean; diffEditor: DiffEditor | null },
): Promise<"reveal" | "quicklook" | "open" | "edit" | "addToPrompt" | "diff" | null> => {
  const actions: SelectItem[] = [
    ...(options.diffEditor
      ? [{ value: "diff", label: `Diff in ${options.diffEditor.label}` }]
      : []),
    { value: "reveal", label: describeRevealAction() },
...
const openDiff = async (
  ...
  if (!diffEditor) {
    ctx.ui.notify("No supported diff editor found (Zed or VS Code)", "warning");
    return;
  }
  ...
  try {
    ...
    const command = buildDiffCommand(diffEditor.executable, tmpFile, workingPath);
    const openResult = await pi.exec(command.executable, command.args, { cwd: gitRoot });
      ...
      handleInput(data: string) {
        if (matchesKey(data, "ctrl+shift+d")) {
          ...
          if (selected) {
            ...
            const canDiff =
              file?.isTracked && !file.isDirectory && Boolean(gitRoot) && Boolean(diffEditor);
            if (!canDiff) {
              ctx.ui.notify(
                "Diff requires a tracked file and an installed Zed or VS Code CLI",
                "warning",
              );
              return;
            }
...
const runFileBrowser = async (pi: ExtensionAPI, ctx: ExtensionContext): Promise<void> => {
  ...
  const diffEditor = selectDiffEditor(findPathExecutables(["zeditor", "zed", "code"]));
  if (files.length === 0) {
  ...
  while (true) {
    ...
    if (quickAction === "diff") {
      await openDiff(pi, ctx, selected, gitRoot, diffEditor);
      continue;
    }
    ...
    const action = await showActionSelector(ctx, {
      ...
      diffEditor: selected.isTracked && !selected.isDirectory && gitRoot ? diffEditor : null,
    });
    if (!action) {
      continue;
    }
